-- Jayna Reviews Database Schema
-- Production-ready schema with comprehensive relationships and constraints
...
-- User notification status (who has seen what)
...
-- Workflow tasks within templates
...
-- Task completions within workflow instances
...
-- Review instances and responses
...
CREATE POLICY notifications_for_user ON user_notifications FOR ALL USING (auth.uid() = user_id);
...
CREATE TRIGGER audit_workflow_instances AFTER INSERT OR UPDATE OR DELETE ON workflow_instances FOR EACH ROW EXECUTE FUNCTION audit_trigger_function();
CREATE TRIGGER audit_review_instances AFTER INSERT OR UPDATE OR DELETE ON review_instances FOR EACH ROW EXECUTE FUNCTION audit_trigger_function();
...
-- Get the review template ID for categories
DO $$
DECLARE
    template_id UUID;
BEGIN
...
('admin@jaynagyro.com', 'System', 'Administrator', 'admin', 'ADMIN001', CURRENT_DATE, true);

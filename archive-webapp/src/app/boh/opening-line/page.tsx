...
export default function BOHOpeningLineWorksheet() {
  ...
  const getTranslatedText = useCallback((id: string, englishText: string) => {
    const translations: any = {
      ...
      tr: {
        ...
        '5': 'Buzdolabı ve dondurucu sıcaklıklarını kontrol edin',
        '6': 'Tüm soğutulmuş ünitelerin uygun sıcaklıkta olduğunu kontrol edin (41°F altında)',
        '7': 'Tüm hazır ürünlerin son kullanma tarihlerini kontrol edin',
        '8': 'FIFO yöntemini kullanarak envanter rotasyonu yapın',
        '9': 'Renk kodlu kesme tahtalarını kurun',
        '10': 'Tüm sos kaplarını doldurun ve konumlandırın',
        '11': 'Hattı taze hazırlık malzemeleri ile stoklayın',
        '12': 'Porsiyon boyutlarını ve kapları kontrol edin',
        '13': 'Tüm çalışma yüzeylerini ve kesme tahtalarını dezenfekte edin',
        '14': 'El yıkama istasyonlarını sabun ve havlularla kurun',
        '15': 'Dezenfektan solüsyon konsantrasyonunu kontrol edin (200-400 ppm)',
        '16': 'Tüm araç ve gereçleri temizleyin ve dezenfekte edin',
  ...
  useEffect(() => {
    ...
    const bohOpeningLineItems: WorksheetItemData[] = [
      ...
      console.error('Error loading BOH opening worksheet:', error);
      setItems([]);
    }
  }, [mounted, language, getTranslatedText]);
  ...
  const handleComplete = () => {
    try {
      ...
      console.error('Error saving BOH worksheet:', error);

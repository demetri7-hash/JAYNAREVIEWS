...
export async function POST() {
  try {
    ...
    const policiesSQL = `
      ...
      DROP POLICY IF EXISTS "Managers can create assignments" ON assignments;
      ...
      DROP POLICY IF EXISTS "Users can view relevant completions" ON completions;
    ...
    if (policiesError) {
      console.warn('Policies setup warning:', policiesError.message)
    }
    ...
    if (indexesError) {
      console.warn('Indexes setup warning:', indexesError.message)
    }

    return NextResponse.json({
      success: true,
      message: 'Database setup completed successfully!',
      tables: ['profiles', 'tasks', 'assignments', 'completions'],
      features: ['RLS enabled', 'Policies created', 'Indexes added']
    })

...
export default function FeaturesOverview() {
  ...
  const handleSetupWorkflows = async () => {
    ...
    try {
      const response = await fetch('/api/setup-workflows', {
        method: 'POST',
          ...
          {!setupComplete ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div className="flex items-start">
                <WrenchScrewdriverIcon className="h-6 w-6 text-yellow-600 mr-3 mt-1" />
                <div className="flex-1">
                  <h3 className="font-medium text-yellow-800 mb-2">
                    Setup Required: Initialize Workflow System
                  </h3>
                  <p className="text-yellow-700 text-sm mb-4">
                    The workflow system needs to be initialized with database tables and sample workflows. 
                    This will create the foundation for all task management features.
                  </p>
                  
                  {setupError && (
                    <div className="bg-red-100 border border-red-200 rounded p-3 mb-4">
                      <p className="text-red-800 text-sm">❌ {setupError}</p>
                    </div>
                  )}
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-green-700 text-sm">
                    Workflow system initialized with sample templates and tasks. You can now use all features!
                  </p>
                </div>
              </div>
            </div>
